...
%  Fault injection
fault_time  = 20;    % time of wheel failure
fault_wheel = 3;     % which wheel fails
...
for i = 1:N
    ...
    ang_d  = waypoints(wp_idx, 1) * pi/180;
    ax_d   = waypoints(wp_idx, 2:4)';
    ax_d   = ax_d / norm(ax_d);
...
pre_idx  = time <  fault_time;
post_idx = time >= fault_time;
...
if fault_time < t_final
    post_err  = err_hist(post_idx);
    post_time = time(post_idx);
    recover_idx = find(post_err < thresh, 1, 'first');
    if ~isempty(recover_idx)
        fprintf('Recovery time after fault: %.2f s\n', post_time(recover_idx) - fault_time);
    else
        fprintf('Recovery: Not achieved after fault within simulation window\n');
    end
end
...
xline(fault_time, '--', 'Color', [0.2 0.2 0.2], 'LineWidth', 1.5);
text(fault_time, max(err_hist)*0.9, ' RW FAILURE', 'Color', 'k', 'FontWeight', 'bold');
title('A. Adaptative Gain Response', 'Color', 'k');
...
title('B. Phase Plane Trajectory', 'Color', 'k');
...
xlabel('Time [s]', 'FontWeight', 'bold');
title('C. Sliding Surface Stability (Convergence Proof)', 'Color', 'k');
